...
def create_cert_operator():
  ...
  with tempfile.TemporaryDirectory() as tempdir:
    ...
    shell(["openssl", "req", "-x509", "-new", "-nodes", "-key", key_path, "-subj", "/CN={}".format(get_lab_name()), "-days", "3650", "-reqexts", "v3_req", "-extensions", "v3_ca", "-out", cert_path])

    with open(key_path, "r") as keyfile:
      with open(cert_path, "r") as certfile:
        kubecfg("cluster-ca.jsonnet", {
          "key": keyfile.read(),
          "cert": certfile.read(),
        })